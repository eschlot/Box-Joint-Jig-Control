...
#ifndef __INDIVIDUALDISTRIBUTIONMENU_H__
#define __INDIVIDUALDISTRIBUTIONMENU_H__
...
class IndividualDistributionMenu : public Menu
{
  //variables
  public:
  protected:
  ...
  LinkedObjectList<GrooveDefinition*> *individualDistribution;

  GrooveDefinition* lastSelectedGrooveDefinition;

  //functions
  public:
  IndividualDistributionMenu(UTFT *Disp, URTouch *Touch,TFT_Extension *TFTextention);

  virtual ~IndividualDistributionMenu(void);
  virtual void dispatch(uint8_t buttonNumber);
  virtual void menuIsActivating(void);

  protected:
  private:
  virtual bool calculateGrooves(void);

  IndividualDistributionMenu( const IndividualDistributionMenu &c );
  IndividualDistributionMenu& operator=( const IndividualDistributionMenu &c );
  void printErrorMessage(char const* message);
  virtual void printGroove(int index,int inLine,boolean selected);
  virtual	void printGrooves();
  virtual void fillOverallLengthButton(void);
}; //IndividualDistributionMenu

#endif //__INDIVIDUALDISTRIBUTIONMENU_H__
